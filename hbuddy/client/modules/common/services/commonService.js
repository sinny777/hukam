...
define(['angular'], function (angular) {
  ...
  var factory = function ($rootScope, $cookieStore, $http) {
	  ...
	  var service = {};
		...
		service.showMessage = function (msgClass, strongMsg, text, $scope){
			console.log('In showMessage: >>>> ' +msgClass+', text: ' +text+', strongMsg: ' +strongMsg);
			$scope.$apply(function(){
				$scope.message = {};
				$scope.message.msgClass = msgClass;
				$scope.message.text = text;
				$scope.message.strongMsg = strongMsg;
			});
		}
		
		service.hideMessage = function ($scope){
			console.log('In showMessage: >>>> ' +msgClass+', text: ' +text+', strongMsg: ' +strongMsg);
			$scope.message = null;
		}
		
		service.isNullOrEmpty = function(elem){
			return (elem == undefined || elem == 'undefined' || elem == null || elem.length == 0);
		}
		...
		service.timeDifference = function(start, end){
		...
		service.handleCarousel = function(divId, nextId, prevId){
			var portfolioCarousel = $(divId);
		...
		service.handleTabWidgets = function(tabid){
		    ...
		    if( window.location.hash != '' ) {
		    	 ...
		    	 console.log('TAB HASH: >>>  ' +getTabHash);
		...
		service.pageLoadCalls2 = function(){
	 // Dropdown Menu
	       ...
	       /* if ( $().tipsy ) { nTip=function(){ $('.ntip').tipsy({gravity: 's', fade:true}); }; nTip(); }
	        ...
	        service.stickyMenuFunction=function(){
	        ...
	        service.topSocialExpander=function(){
	        ...
	        service.siblingsFader=function(){
	        ...
	        if ( $().prettyPhoto ) {
	        	...
	        	service.initprettyPhoto=function(){
	                
	                $("a[rel^='prettyPhoto']").prettyPhoto({ theme: 'light_square', social_tools: false });
	            
	            };
	            service.initprettyPhoto();
	        ...
	        if( $().jflickrfeed ) {
	            ...
	            $('.flickrfeed').each(function() {
	                ...
	                $(this).jflickrfeed({
	            		...
	            		if ( $().prettyPhoto ) { service.initprettyPhoto(); }
	            	});
